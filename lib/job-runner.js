...
const { ensureLegacyBaseline, createRevision } = require('./annotation-history');
const { touchAfterAnnotation } = require('./review-state');
...
function createInferenceJob({ userId, projectId, modelId, imageIds, selection = 'unannotated', confThreshold = 0.25, goodBias = 0.5, clsModelId, clsFineModelId, clsCombinedModelId, replaceExisting = false, name }) {
  ...
  if (!modelAccessibleToUser(model, userId, projectId)) throw new Error('The selected model is not owned by you or shared with project collaborators.');
  for (const [field, id] of Object.entries({ clsModelId, clsFineModelId, clsCombinedModelId })) {
    ...
    if (!modelAccessibleToUser(auxiliary, userId, projectId)) throw new Error(`${field} is not shared with project collaborators.`);
  }
...
function writeInferenceAnnotations(job, item, inference) {
  ...
  const allAnnotations = readJson(annotationsFile, []);
  const existing = allAnnotations.filter(annotation => annotation.imageId === item.imageId);
  const previousJobAnnotations = existing.filter(annotation =>
    annotation.jobId === job.id || annotation.provenance?.jobId === job.id,
  );
  const results = Array.isArray(inference?.results) ? inference.results : [];

  // An empty model response is not an annotation edit. In particular, do not
  // turn an already reviewed image into a submitted (or unannotated) image.
  ...
  else annotations = annotations.filter(annotation => !(annotation.imageId === item.imageId && (
    annotation.jobId === job.id || annotation.provenance?.jobId === job.id
  )));

  const inferredAt = nowIso();
  const actorUsername = job.userUsername || 'Automation';
  const usedIds = new Set(annotations.map(annotation => annotation.id));
  const generated = results.map((shape, index) => {
    const previous = previousJobAnnotations.find(annotation =>
      Number(annotation.resultIndex) === index && !usedIds.has(annotation.id),
    );
    let id = typeof shape.id === 'string' && /^[a-zA-Z0-9_-]{1,128}$/.test(shape.id)
      ? shape.id
      : (previous?.id || uuidv4());
    if (usedIds.has(id)) id = uuidv4();
    usedIds.add(id);
    const rawConfidence = shape.confidence ?? shape.conf;
    ...
    return {
      ...
      jobId: job.id,
      modelName: inference.model?.name || null,
      modelFormat: inference.model?.format || null,
      ...
      createdAt: previous?.createdAt || inferredAt,
      updatedAt: inferredAt,
      updatedBy: job.userId,
      updatedByUsername: actorUsername,
    };
  });
  const finalAnnotations = [...annotations, ...generated];
  writeJson(annotationsFile, finalAnnotations);
  ...
  if (image) {
    const imageAnnotations = finalAnnotations.filter(annotation => annotation.imageId === item.imageId);
    image.annotated = imageAnnotations.length > 0 || Boolean(image.isNull);
    ...
    reviewUpdate = touchAfterAnnotation(image, imageAnnotations.length, job.userId, actorUsername);
    appendAuditEvent({
      projectId: job.projectId,
      imageId: item.imageId,
      actorId: job.userId,
      ...
      details: {
        ...
        modelId: job.input.modelId,
        annotationCount: generated.length,
        totalAnnotationCount: imageAnnotations.length,
...
async function runInferenceJob(jobId, controller) {
  ...
  for (const item of job.items || []) {
    ...
    try {
      ...
      const reviewStatus = readJson(dataFile('images.json'), [])
        .find(image => image.id === item.imageId)?.reviewStatus || null;
