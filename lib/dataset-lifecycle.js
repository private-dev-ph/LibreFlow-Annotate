...
const { materializeVersionInputs } = require('./version-processing');
...
function geometryProblems(annotation, image) {
  ...
  if (annotation.type === 'bbox') {
    ...
    const points = Array.isArray(data) ? data : (Array.isArray(data.points) ? data.points : []);
    if (points.length < 3) add('invalid_geometry', 'Polygon must contain at least three points.');
    points.forEach((p, index) => {
      const x = Number(p.x), y = Number(p.y);
      if (!Number.isFinite(x) || !Number.isFinite(y)) add('invalid_geometry', `Polygon point ${index + 1} is not numeric.`);
      else if (width && height && (x < 0 || y < 0 || x > width || y > height)) add('out_of_bounds', `Polygon point ${index + 1} is outside the image.`);
    });
...
function analyzeHealth(input = {}) {
  ...
  return {
    ...
    boundingBoxes: {
      count: annotations.filter(a => a.type === 'bbox').length,
...
function normalizeAnnotation(annotation) {
  const out = {
    ...
    data: canonicalize(annotation.data),
  };
  ['confidence', 'createdAt', 'createdBy', 'source', 'modelId', 'modelVersion', 'import'].forEach(key => {
    if (annotation[key] !== undefined) out[key] = canonicalize(annotation[key]);
  });
  return out;
}
